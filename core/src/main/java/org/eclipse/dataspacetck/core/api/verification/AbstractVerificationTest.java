...
public abstract class AbstractVerificationTest {
    ...
    private static final JsonSchemaFactory schemaFactory = JsonSchemaFactory.getInstance(V202012, builder ->
            builder.schemaMappers(schemaMappers ->
                    schemaMappers.mapPrefix(DSPACE_NAMESPACE + "/", CLASSPATH_SCHEMA))
    );


    static protected MessageValidator forSchema(String schema) {
        return (input) -> {
